...
def render_board_md(board: list, lang_key: str, owner: str, repo: str,
    ...
    rows = ['|   | 1 | 2 | 3 |', '|---|---|---|---|']
    for ri, row_label in enumerate(['A', 'B', 'C']):
        cells = []
        for ci in range(3):
            val = board[ri][ci]
            cell_name = f'{row_label}{ci+1}'
            if val:
                cells.append(SYMBOLS[val])
            elif winner:
                cells.append('➕')
            else:
                link = f'https://github.com/{owner}/{repo}/issues/new?title={lang_display}%3A+Tic-Tac-Toe%3A+Put+{cell_name}&body=Play+{lang_display}+board'
                cells.append(f'[➕]({link})')
        rows.append(f'| **{row_label}** | {" | ".join(cells)} |')
    board_md = '\n'.join(rows)

    if winner:
        status = f'**{SYMBOLS[winner]} {winner} wins!** — [New game / Reset](https://github.com/{owner}/{repo}/issues/new?title={lang_display}%3A+Tic-Tac-Toe%3A+Reset&body=Reset+the+board)'
    elif is_draw(board):
        status = f"**It's a draw!** — [New game / Reset](https://github.com/{owner}/{repo}/issues/new?title={lang_display}%3A+Tic-Tac-Toe%3A+Reset&body=Reset+the+board)"
    else:
